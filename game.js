...
function scoreHand(localHand) {
    if (gameVars.currentHands > 0 && localHand.length > 0) {
        ...
        // if a hand is allowed by a boss blind
        if (!gameVars.notAllowed) {
            ...
            // in the case that a hand isn't allowed
            currentScore = { chips: 0, mult: 0 }
        }
        ...
        if (gameVars.score >= gameVars.neededScore) {
            ...
            handleBossAbility(gameVars.blind, 'roundEnd');
...
function handleBossAbility(blind, trigger) {
    if (blind.modifyTrigger === trigger) {
        if ('effect' in blind) {
            if ('modifyCondition' in blind) {
                if (blind.modifyCondition()) {
                    blind.modifyEffect();
                }
            } else {
                blind.modifyEffect();
...
function handleCard(card, retrigger) {
    // if the card is debuffed don't do anything
    if (!card.debuffed) {
...
// deals a hand
function dealHand() {
    // hand is by default allowed after a hand is dealt
    gameVars.notAllowed = false;
    ...
    for (i = hand.length; i < gameVars.handSize; i++) {
        ...
        if (deck.filter(c => !c.dealt).length > 0) {
            ...
            if (deck[getCardIndex(id)] && !deck[getCardIndex(id)].dealt) {
                ...
                drawnCard = deck[getCardIndex(id)];

                if (gameVars.blind.boss) {
                    handleBossAbility(gameVars.blind, 'onDraw');
                }
            } else {
...
// also holds the code for rendering the card to HTML
function sortHand(type) {
    ...
    if (type === 'rank') {
        ...
        hand.sort(function (a, b) {
            if (a.enhancement === 'stone') {
                return 1
            } else if (b.enhancement === 'stone') {
                return -1;
            }
    ...
    for (i = 0; i < hand.length; i++) {
        ...
        if (hand[i].selected) {
            ...
            template = `<button class="card" id="card-${i}" onclick="function toggleSelectSelf() {
            if (hand.filter(c => c.selected).length < 5 && !hand[${i}].selected) {
                document.getElementById('card-${i}').classList.add('selected');
                hand[${i}].selected = true;
            } else if (hand[${i}].selected) {
        ...
        if (!hand[i].faceDown) {
            ...
            displayHand.innerHTML += `${template}Facedown</p></button>`
        }
    }
}
...
function runRound(blind, setScore) {
    ...
    console.log(`Hands left: ${gameVars.currentHands}\nDiscards left: ${gameVars.currentDiscards}`);
